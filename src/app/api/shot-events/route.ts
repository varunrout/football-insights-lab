...
export async function GET() {
  ...
  try {
    if (appEnv !== 'TEST') {
      ...
      const backendUrl = `${pythonBackendBaseUrl}/api/shot_events`; // Assuming Python endpoint is /api/shot-events
    ...
    console.error('Error in Next.js /api/shot-events route:', error);
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json({ message: 'Error fetching shot events data in Next.js route', error: errorMessage }, { status: 500 });
  }
}
