...
export async function GET() {
  ...
  try {
    if (appEnv !== 'TEST') {
      ...
      const backendUrl = `${pythonBackendBaseUrl}/api/players`; // Assuming Python endpoint is /api/players
    ...
    console.error('Error in Next.js /api/players route:', error);
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json({ message: 'Error fetching players data in Next.js route', error: errorMessage }, { status: 500 });
  }
}
