...
export async function GET() {
  ...
  try {
    if (appEnv !== 'TEST') {
      ...
      const backendUrl = `${pythonBackendBaseUrl}/api/player_stats`; // Assuming Python endpoint is /api/player-stats
    ...
    console.error('Error in Next.js /api/player-stats route:', error);
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json({ message: 'Error fetching player stats data in Next.js route', error: errorMessage }, { status: 500 });
  }
}
